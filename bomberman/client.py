...
class Client(object):	
	...
	def is_profane(self, corpus):
		...
		self.conn.request("GET", "/v1/profanity/check?%s" % params, {}, self.conn.headers)
	...
	def censor(self, corpus, replacement_text="***"):
		...
		self.conn.request("GET", "/v1/profanity/censor?%s" % params, {}, self.conn.headers)
		resp = self.conn.getresponse()
		if resp.status == 200:
	...
	def highlight(self, corpus, start_tag="<strong>", end_tag="</strong>"):
		...
		self.conn.request("GET", "/v1/profanity/highlight?%s" % params, {}, self.conn.headers)
		resp = self.conn.getresponse()
		if resp.status == 200:
			data = json.loads(resp.read())
			self.conn.close()
